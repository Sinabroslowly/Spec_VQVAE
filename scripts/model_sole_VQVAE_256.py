...
class Encoder(nn.Module):
    ...
    def build_model(self): # [batch, 1, 512, 512]
        model = []
        model.append(nn.Conv2d(1, 256, kernel_size=4, stride=2, padding=1, bias=False))
        ...
        model.append(EqualizedLearningRateLayer(model[-1]))
        model.append(nn.LeakyReLU(negative_slope=0.2)),
        model.append(PixelWiseNormLayer()) # [batch, 256, 256, 256]
        ...
        model.append(EqualizedLearningRateLayer(model[-1]))
        model.append(nn.LeakyReLU(negative_slope=0.2)),
        model.append(PixelWiseNormLayer()) # [batch, 128, 128, 128]
        ...
        model.append(EqualizedLearningRateLayer(model[-1]))
        model.append(nn.LeakyReLU(negative_slope=0.2)),
        model.append(PixelWiseNormLayer()) # [batch, 64, 64, 64]
        ...
        model.append(EqualizedLearningRateLayer(model[-1]))
        model.append(nn.LeakyReLU(negative_slope=0.2)),
        model.append(PixelWiseNormLayer()) # [batch, 32, 64, 64]
        
        model.append(nn.Conv2d(32, self.embedding_dim, kernel_size=1, stride=1, padding=0, bias=False))
        model.append(EqualizedLearningRateLayer(model[-1]))
        model.append(nn.Tanh()) # [bach, embedding_dim, 64, 64]
...
class Decoder(nn.Module):
    ...
    def build_model(self): # [batch, embedding_dim, 64, 64]
        ...
        model.append(nn.ConvTranspose2d(129, 256, kernel_size=3, stride=1, padding=1, bias=False)) 
        model.append(EqualizedLearningRateLayer(model[-1]))
        ...
        model.append(nn.ConvTranspose2d(256, 1, kernel_size=4, stride=2, padding=1, bias=False)) # [batch, 1, 512, 512]
...
class VQVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        quantized, quantization_loss = self.quantizer(z_e)  # Quantize with diffusers.VQModel
